...
router.get('/:id', authenticateToken, async (req, res) => {
  const tournament = await Tournament.findById(req.params.id).populate('players.userId matches.playerA matches.playerB');
...
router.post('/:id/matches/:matchId/resolve', authenticateToken, async (req, res) => {
  const { action, scoreA, scoreB } = req.body;
  const tournament = await Tournament.findById(req.params.id);
  if (!tournament) return res.status(404).json({ error: 'Турнир не найден' });
  const match = tournament.matches.id(req.params.matchId);
  if (!match || match.status !== 'disputed') return res.status(400).json({ error: 'Нет спора для этого матча' });
  if (action === 'approve') {
    ...
    await tournament.save();
    await tryFinalize(tournament);
    return res.json({ message: 'Спор одобрен и результат подтверждён', match });
  }
  if (action === 'reject') {
    match.status = 'pending';
    delete match.dispute;
    await tournament.save();
    return res.json({ message: 'Спор отклонён — результат сброшен', match });
  }
  res.status(400).json({ error: 'Действие должно быть approve или reject' });
});
